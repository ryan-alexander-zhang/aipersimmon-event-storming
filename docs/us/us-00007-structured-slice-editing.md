...
- **us-00007-FR-1** (Event) When the Modeler uses a slice action on a selected
  Domain Event or Command (+ Command, + Actor, + Aggregate, + Policy,
  + Read Model, + Hotspot), the system shall create the element with the
  grammar-correct relation and place it in its row-band within the slice's
  column.
